...
/**
************************************************************************
* @brief:      	dm8009_data: 获取DM8009电机反馈数据函数
...
void Joint_Motor_pos_control(float pos){
		...
		switch(Selection)
		{
			case 0:
			{
				DM_Motor_mit_ctrl(&hcan1,DM8009_1_CAN_ID, degreesToRadians(-pos), 0,DM8009_1_POS_KP, DM8009_1_POS_KD, 0);	  //IMT控制模式对ID为MOTOR_1_CAN_ID电机进行速度控制发送
				Selection++;
				break;
			}
			case 1:
			{
				DM_Motor_mit_ctrl(&hcan1,DM8009_2_CAN_ID, degreesToRadians(-10), 0,DM8009_2_POS_KP, DM8009_2_POS_KD, 0);			//IMT控制模式对ID为MOTOR_2_CAN_ID电机进行速度控制发送
				Selection++;
				break;
			}
			case 2:
			{
				DM_Motor_mit_ctrl(&hcan1,DM8009_3_CAN_ID, degreesToRadians(-pos), 0,DM8009_3_POS_KP, DM8009_3_POS_KD, 0);			//IMT控制模式对ID为MOTOR_3_CAN_ID电机进行速度控制发送
				Selection++;
				break;
			}
			case 3:
			{
				DM_Motor_mit_ctrl(&hcan1,DM8009_4_CAN_ID, degreesToRadians(pos), 0,DM8009_4_POS_KP, DM8009_4_POS_KD, 0);			//IMT控制模式对ID为MOTOR_4_CAN_ID电机进行速度控制发送
				Selection=0;
				break;
			}
		}
		...
		switch(Selection)
		{
			case 0:
			{
				PosSpeed_CtrlMotor(&hcan1,0X102, 10, 5);	  //位置速度控制模式对ID为0X01电机进行速度控制发送
				Selection++;
				break;
			}
			case 1:
			...
			case 2:
			{
				PosSpeed_CtrlMotor(&hcan1,0X104,  10, 5);			//位置速度控制模式对ID为0x05电机进行速度控制发送
				Selection=0;
				break;
			}
		}
		...
		switch(Selection)
		{
			case 0:
			{
				Speed_CtrlMotor(&hcan1,0X202, 5);	  //速度控制模式对ID为0X01电机进行速度控制发送
				Selection++;
				break;
			}
			case 1:
			...
			case 2:
			{
				Speed_CtrlMotor(&hcan1,0X204, 5);			//速度控制模式对ID为0x05电机进行速度控制发送
				Selection=0;
				break;
			}
		}
