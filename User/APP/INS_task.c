...
void IMU_Init_Self_inspection(void);
...
/**
  * @brief          Initialization of imu related peripherals, including gyroscopes and magnetometers, 
										and initialization of PID and AHRS algorithms, 
										as well as peripherals such as SPIDMA reception,
  * @param[in]      pvParameters: NULL
  * @retval         none
  */
/**
  * @brief          imu相关外设初始化, 包括陀螺仪与磁力计，以及PID和AHRS算法相关的初始化，以及SPIDMA接收等外设,
  * @param[in]      pvParameters: NULL
  * @retval         none
  */
void IMU_Init_Self_inspection(void)
{
    ...
    //osDelay(INS_TASK_INIT_TIME);
		printf("INS_task_start\r\n");
    while(BMI088_init())	//BMI088初始化,若初始化不成功打印错误信息
    {
			printf("ERROR:BMI088_init\r\n");
        osDelay(100);
    }
    while(ist8310_init())	//ist8310初始化,若初始化不成功打印错误信息
    {
			printf("ERROR:ist8310_init\r\n");
        osDelay(100);
    }
    ...
    imu_start_dma_flag = 1;	//IMU的dma接收使能
}


/**
  * @brief          The imu continuously processes the data during the task and updates the Angle ->get_angle
  * @param[in]      NULL
  * @retval         none
  */
/**
  * @brief          imu在任务中不断处理数据,并且更新角度->get_angle
  * @param[in]      NULL
  * @retval         none
  */
void IMU_Data_Process(void)
